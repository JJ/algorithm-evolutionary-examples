...
=head1 NAME
  ...
  anyfitness-noisy-memory-w-lted.pl - Massively multimodal deceptive problem

=head1 SYNOPSIS

  prompt% ./anyfitness-noisy-memory-w-lted.pl conf.yaml
...
# Open output stream
#----------------------------
my $ID="res-lafnmw-".$conf->{'fitness'}->{'class'}."-p". $population_size."-ns". $noise_sigma."-cs".$chromosome_length."-rr".$replacement_rate."-im".$initial_memory;
